...
feature -- Access
	...
	selected_game_index: INTEGER
			-- The index of the game selected in the dialog.
		require
			not_cancelled: not is_cancelled
		do
			check attached game_list.selected_item as s then
				check attached {TUPLE [i: INTEGER_REF; dt: DATE_TIME]} s.data as d then
					Result := d.i.item
				end
			end
		end
...
feature -- Element change
	...
	fill_game_list (a_list: LINKED_LIST [TUPLE [i: INTEGER_REF; dt: DATE_TIME]])
			-- Put the persisted games, identified in `a_list' into Current.
			-- I used {DATE_TIME} instead of my {YMDHMS_TIME} to conform to
			-- EiffelStore's known types.  (See `loaded_game_index_tuples'
			-- from {FLIPPER_MAIN_WINDOW}.
		local
			list_i: EV_LIST_ITEM
		do
			from a_list.start
			...
			loop
				create list_i.make_with_text (a_list.item.dt.out)
